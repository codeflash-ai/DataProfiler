...
class StructuredProfiler(BaseProfiler):
    ...
    def _get_duplicate_row_count(self) -> int | None:
        ...
        # Local variable for options.row_statistics to accelerate attribute lookup
        options_row_stats = self.options.row_statistics
        # Check disabled states up front (fast path)
        if (
            not options_row_stats.is_enabled
            or not options_row_stats.unique_count.is_enabled
        ):
            return None

        hr_obj = self.hashed_row_object

        # isinstance(dict) is fast, elif right after, reduces unnecessary checks
        if type(hr_obj) is dict:
            # Since dict() is default, check type directly (faster than isinstance)
            # total_samples is always int, len(hr_obj) is fast
            return self.total_samples - len(hr_obj)
        elif type(hr_obj) is HyperLogLog:
            # Only call cardinality when HyperLogLog is actually used
            # Cache arithmetic in local vars to minimize attribute lookups
            card = hr_obj.cardinality()
            ts = self.total_samples
            diff = ts - int(card)
            return diff if diff > 0 else 0
        # Else fallback for other types (shouldn't occur)
        return 0
